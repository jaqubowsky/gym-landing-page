...
const Burger: FC<Burger> = ({ isOpen, onClick }) => {
  return (
    ...
    >
      <div
        className={`relative bg-white h-1 w-8 transition-all duration-300 delay-75 filter ease-in-out will-change-transform" ${
          isOpen ? "bg-yellow-300 transform rotate-45 translate-y-[12px]" : "will-change-auto"
        }`}
      />
      <div
        className={`relative bg-white h-1 w-8 opacity-1 transition-all duration-300 delay-75 filter ease-in-out will-change-transform" ${
          isOpen ? "bg-yellow-300 opacity-0" : "will-change-auto"
        }`}
      />
      <div
        className={`relative bg-white h-1 w-8 transition-all duration-200 delay-75 filter ease-in-out will-change-transform ${
